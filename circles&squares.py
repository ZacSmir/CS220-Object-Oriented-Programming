...
class Shape:
    def __init__(self, x, y, a, b, j, k):
        ...
        self.outlinecolor = '#{0:0>6x}'.format(random.randint(00,16**6))
...
class Square(Shape):
    def draw(self, canvas):
        canvas.create_rectangle(self.x, self.y, self.x + self.size, self.y + self.size,fill=self.color, outline=self.outlinecolor)
    
class Circle(Shape):
    def draw(self, canvas):
        canvas.create_oval(self.x, self.y, self.x + self.size, self.y + self.size,fill=self.color, outline=self.outlinecolor)
...
def addSquare(event):
    ...
    global game_objects
    game_objects.append(Square(250, 250, -((250-event.x)/100.0), -((250-event.y)/100.0), event.x, event.y ))
...
def draw(canvas):
    ...
    #to make it glitchy looking but to keep it from lagging, nex 2 lines \/
